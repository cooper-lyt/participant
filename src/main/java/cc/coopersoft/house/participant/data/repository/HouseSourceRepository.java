...
public interface HouseSourceRepository extends EntityRepository<HouseSource,String> {
    ...
    @Query("select hsc from HouseSourceCompany hsc left join fetch hsc.houseSource hs left join fetch hs.houseSaleInfo hsi left join fetch hs.proxyPerson pp left join hs.houseContract hc  where hsc.groupId = :groupId and (hs.id = :condition or hs.houseCode = :condition or hs.sourceId = :condition  or hc.id = :condition  or hs.searchKey like :likeCondition or hsc.title like :likeCondition or false = :hasCondition) and hs.status in (:statuses) and (hs.applyTime <= :endDate or false = :hasEndDate) and (hs.applyTime >= :startDate or false = :hasStartDate) order by hs.updateTime desc, hs.sourceId")
    ...
    @Query("select count(hsc) from HouseSourceCompany hsc left join hsc.houseSource hs left join hs.proxyPerson pp left join hs.houseContract hc where hsc.groupId = :groupId and (hs.id = :condition or hs.houseCode = :condition or hs.sourceId = :condition or hc.id = :condition or hs.searchKey like :likeCondition or hsc.title like :likeCondition or false = :hasCondition) and hs.status in (:statuses) and (hs.applyTime <= :endDate or false = :hasEndDate) and (hs.applyTime >= :startDate or false = :hasStartDate) order by hs.updateTime desc")
    Long searchResultCount(@QueryParam("groupId") String groupId, @QueryParam("condition") String condition, @QueryParam("likeCondition") String likeCondition, @QueryParam("hasCondition") boolean hasCondition, @QueryParam("startDate") Date startDate, @QueryParam("hasStartDate") boolean hasStartDate , @QueryParam("endDate") Date endDate, @QueryParam("hasEndDate") boolean hasEndDate,@QueryParam("statuses") Set<HouseSource.HouseSourceStatus> statuses);

    @Query("select new cc.coopersoft.house.participant.data.model.HouseFilterType(hs.status,count(hs.id)) from HouseSourceCompany hsc left join hsc.houseSource hs left join hs.proxyPerson pp left join hs.houseContract hc where hsc.groupId = :groupId and (hs.id = :condition or hs.houseCode = :condition or hs.sourceId = :condition or hc.id = :condition or hs.searchKey like :likeCondition or hsc.title like :likeCondition or false = :hasCondition) and (hs.applyTime <= :endDate or false = :hasEndDate) and (hs.applyTime >= :startDate or false = :hasStartDate) group by hs.status")
